...
d3.csv("./data/data.csv", function(error, mojoData) {
  ...
  // Scale the domain
  xTimeScale.domain(d3.extent(mojoData, function(data) {
    return data.date;
  }));
  yLinearScale1.domain([d3.min(mojoData, function(data) {
    return data.dow_index;
  }), d3.max(mojoData, function(data) {
    return data.dow_index;
  })]);
  yLinearScale2.domain([d3.min(mojoData, function(data) {
